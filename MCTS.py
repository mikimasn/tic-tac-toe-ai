        ...
        while not actNodeHash == -1:
            ...
            for idx, hash in enumerate(actNode.possibleMovesHash):
                ...
                if hash == -1:
                    ...
                    value = actNode.possibleMovesEvalSum[idx] / actNode.possibleMovesVisitCount[idx] + self.cpuct * \
...
class MCTSTickTacToe(MCTSForestParticipant[GameState, int]):
    ...
    def _apply_move(self, gameState: GameState, move: int) -> GameState:
        gameState = GameState(not gameState.crossNowPlaying, gameState.nowallowed, gameState.board.copy(),
        ...
        subboard, submove = (move // 27) * 3 + (move // 3) % 3, (move // 9) % 3 + move % 3
